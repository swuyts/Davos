...
# Adapted to be in line with the 2015 additions of Goldman et al.
...
class DNA(object):
    ...
    def __S4_to_S0(self, s4):
        ...
        # s2 is the last 20 trits | UPDATE: In newest version s2 is formed by the first 25 trits
        s2 = s4[:25]

        # n = len(s1) is s2 converted to base 10 
        n = self.__base3_to_base10(s2)

        # s1 is the first n trits | UPDATE: In newest version S1 is squeezed in between S2 and S3. 
        s1 = s4[25:n+25]
